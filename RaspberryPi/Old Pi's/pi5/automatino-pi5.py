...
LED_HIGH = 12
LED_DIM = 23
BUTTON_OUT = 4 # always high, gp17 is the read pin
# input pins in GPIO
BUTTON_IN = 17
DHT_PIN = 2
RES_HIGH_READ = 27
...
GPIO.output(LED_HIGH, False)
GPIO.output(INTAKE_FAN, False)
GPIO.output(HEAT_PIN, False)
GPIO.output(EXHAUST_FAN, False)
...
light_pwm = GPIO.PWM(LED_HIGH, 100)
light_pwm.start(0)
fan_pwm = GPIO.PWM(INTAKE_FAN, 100);
fan_pwm.start(0)
heat_pwm = GPIO.PWM(HEAT_PIN, 100);
heat_pwm.start(0)
exhaust_pwm = GPIO.PWM(EXHAUST_FAN, 100);
exhaust_pwm.start(0)
