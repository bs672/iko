...
class NewVitalsController: UIViewController {
    ...
    var waterLabelValue: String {
        get {
            return waterLabel.text!
        }
        set {
            waterLabel.text = "Water: \(newValue)"
        }
    }
...
//                Do something
    ...
    func updateStatus(payload json: JSON) {
        ...
        if let waterReported = json["state"]["reported"]["water"].int {
            switch waterReported {
            case 2:
                waterLabelValue = "High"
            case 1:
                waterLabelValue = "Medium"
            case 0:
                waterLabelValue = "Low"
            default:
                break
